package com.mateuszholik.domain.usecase

import com.mateuszholik.data.repositories.PasswordsRepository
import com.mateuszholik.data.repositories.models.Password
import com.mateuszholik.domain.mappers.PasswordsListToPasswordsTypeListMapper
import com.mateuszholik.domain.models.PasswordType
import io.mockk.every
import io.mockk.mockk
import io.reactivex.rxjava3.core.Single
import org.junit.jupiter.api.Test
import java.time.LocalDateTime

class GetPasswordsUseCaseImplTest {

    private val passwordsRepository = mockk<PasswordsRepository> {
        every { getAllPasswords() } returns Single.just(listOf(PASSWORD))
    }

    private val passwordsListToPasswordsTypeListMapper = mockk<PasswordsListToPasswordsTypeListMapper> {
        every { map(listOf(PASSWORD)) } returns listOf(VALID_PASSWORD)
    }

    private val getPasswordUseCase = GetPasswordsUseCaseImpl(
        passwordsRepository = passwordsRepository,
        passwordsListToPasswordsTypeListMapper = passwordsListToPasswordsTypeListMapper
    )

    @Test
    fun `When repository returns all passwords use case will return them correctly`() {
        getPasswordUseCase.invoke()
            .test()
            .assertComplete()
            .assertValue(listOf(VALID_PASSWORD))
    }

    private companion object {
        val PASSWORD = Password(
            id = 0L,
            platformName = "platform",
            password = "password",
            expiringDate = LocalDateTime.of(2022, 6, 11, 12, 0, 0)
        )
        val VALID_PASSWORD = PasswordType.ValidPassword(PASSWORD)
    }
}